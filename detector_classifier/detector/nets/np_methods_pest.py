...
def ssd_bboxes_select_layer(predictions_layer,
    ...
    p_shape_category = predictions_layer_category.shape
    batch_size = p_shape[0] if len(p_shape) == 5 else 1
    predictions_layer = np.reshape(predictions_layer,
                                   (batch_size, -1, p_shape[-1]))
    predictions_layer_category = np.reshape(predictions_layer_category,
                                   (batch_size, -1, p_shape_category[-1]))

    l_shape = localizations_layer.shape
    localizations_layer = np.reshape(localizations_layer,
                                     (batch_size, -1, l_shape[-1]))
    # print "predictions_layer.shape = {}".format(predictions_layer.shape)
    # print "predictions_layer_category.shape = {}".format(predictions_layer_category.shape)
    ...
    # print "sub_predictions = {}".format(sub_predictions.shape)
    idxes = np.where(sub_predictions > select_threshold)
    # print "idxes.shape = {}".format(idxes)
    classes = idxes[-1]+1 # 这里应该是是否是前景的类别 没什么用
    scores = sub_predictions[idxes] #这里是前景背景的置信度
    bboxes = localizations_layer[idxes[:-1]] # 前景置信度大于阈值的框的值

    classes_category = np.argmax(predictions_layer_category[idxes[:-1]],1)
    classes_category = np.expand_dims(classes_category,1)
    scores_category = np.amax(predictions_layer_category[idxes[:-1]], axis=1)
    scores_category = np.expand_dims(scores_category, 1)

    return classes,classes_category,scores_category, scores, bboxes


def ssd_bboxes_select(predictions_net,
                      predictions_net_category,
                      localizations_net,
                      anchors_net,
                      select_threshold=0.5,
                      img_shape=(300, 300),
    ...
    # l_layers = []
    # l_idxes = []
    # print "==============ssd_bboxes_select begin=================="
    for i in range(len(predictions_net)):
        classes, category, category_scores, scores, bboxes = ssd_bboxes_select_layer(
            predictions_net[i], predictions_net_category[i], localizations_net[i], anchors_net[i],
            select_threshold, img_shape, num_classes, decode)
        l_classes.append(classes)
        l_scores.append(scores)
        l_bboxes.append(bboxes)
        # print "bboxes.shape = {}".format(bboxes.shape)
        l_classes_category.append(category)
        l_scores_category.append(category_scores)
        # Debug information.
        # l_layers.append(i)
        # l_idxes.append((i, idxes))

    l_classes_category = np.concatenate(l_classes_category, 0)
    l_scores_category = np.concatenate(l_scores_category, 0)
    # print "l_bboxes = {}".format(l_bboxes)
    classes = np.concatenate(l_classes, 0)
    scores = np.concatenate(l_scores, 0)
    bboxes = np.concatenate(l_bboxes, 0)
    # print "==============ssd_bboxes_select end=================="
    return classes, l_classes_category, l_scores_category, scores, bboxes


# =========================================================================== #
# Common functions for bboxes handling and selection.
# =========================================================================== #
def bboxes_sort(classes, scores, bboxes, rclasses_category, rscores_category, top_k=400):
    """Sort bounding boxes by decreasing order and keep only the top_k
    """
    # if priority_inside:
    #     inside = (bboxes[:, 0] > margin) & (bboxes[:, 1] > margin) & \
    #         (bboxes[:, 2] < 1-margin) & (bboxes[:, 3.txt] < 1-margin)
    ...
    rclasses_category = rclasses_category[idxes][:top_k]
    rscores_category = rscores_category[idxes][:top_k]
    return classes, scores, bboxes, rclasses_category, rscores_category


def bboxes_clip(bbox_ref, bboxes):
    """Clip bounding boxes with respect to reference bbox.
    """
    # print "bboxes - begin = {}".format(bboxes)
    ...
    bboxes = np.transpose(bboxes)
    # print "bboxes - end = {}".format(bboxes)
    return bboxes
...
def bboxes_nms(classes, scores, bboxes, rclasses_category, rscores_category, nms_threshold=0.45):
    ...
    return classes[idxes], scores[idxes], bboxes[idxes], rclasses_category[idxes], rscores_category[idxes]
