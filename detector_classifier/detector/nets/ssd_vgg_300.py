...
def ssd_multibox_layer(inputs,#(end_points[layer]
    ...
    '''
    loc_pred = slim.conv2d(net, 256, [3, 3],
                           scope='conv_loc_pre')
    loc_pred = slim.conv2d(loc_pred, num_loc_pred, [3, 3], activation_fn=None,
                           scope='conv_loc')

    loc_pred = custom_layers.channel_to_last(loc_pred) # 把通道数放到最后 exg: shape = 4×64×64×16
    ...
    num_cls_pred = num_anchors * (num_classes) # 用于分类 每个框回归出num_class个类别  所有有这些个需要回归的数

    cls_pred = slim.conv2d(net, 256, [3, 3],
                           scope='conv_cls_pre')
    cls_pred = slim.conv2d(cls_pred, num_cls_pred, [3, 3], activation_fn=None,
                           scope='conv_cls')
    # mhw add
    pest_pred = slim.conv2d(net, 256, [3, 3],
                           scope='pest_pred_pre')
    pest_pred = slim.conv2d(pest_pred, num_anchors*2, [3, 3], activation_fn=None,
                           scope='pest_pred_cls')
