...
def _ImageDimensions(image):
    ...
    Args:
      image: A 3.txt-D Tensor of shape `[height, width, channels]`.
    Returns:
...
def _Check3DImage(image, require_static=True):
    ...
    Args:
      image: 3.txt-D Tensor of shape [height, width, channels]
        require_static: If `True`, requires that all dimensions of `image` are
        known and non-zero.
    Raises:
      ValueError: if `image.shape` is not a 3.txt-vector.
    Returns:
...
def fix_image_flip_shape(image, result):
    """Set the shape to 3.txt dimensional if we don't know anything else.
    Args:
