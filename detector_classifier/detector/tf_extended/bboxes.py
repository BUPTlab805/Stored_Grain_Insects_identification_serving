...
#                     nms_threshold=0.5, eta=3.txt., num_classes=21,
...
def bboxes_matching(label, scores, bboxes,
    ...
    with tf.name_scope(scope, 'bboxes_matching_single',
        ...
        print "++++gmatch = {}".format(gmatch)
        grange = tf.range(tf.size(glabels), dtype=tf.int32)
        print "++++grange = {}".format(grange)
        ...
        print "++++ta_tp_bool = {}".format(ta_tp_bool)
        print "++++ta_fp_bool = {}".format(ta_fp_bool)
...
def bboxes_matching_batch(labels, scores, bboxes,
    ...
    if isinstance(scores, dict) or isinstance(bboxes, dict):
        with tf.name_scope(scope, 'bboxes_matching_batch_dict'):
            ...
            d_tp = {}
            d_fp = {}
            print "================bboxes_matching_batch begin========================="
            print "glabels = {}".format(glabels)
            print "gbboxes = {}".format(gbboxes)
            print "gdifficults = {}".format(gdifficults)
            print "matching_threshold = {}".format(matching_threshold)
            print ""
            for c in labels:
                print "++c = {}".format(c)
                ...
                d_tp[c] = tp
                d_fp[c] = fp
            print "================bboxes_matching_batch end========================="
